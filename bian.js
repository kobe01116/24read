...
function 開始() {
  ...
  threads.start(function () {
    while (true) {
      ...
      if (Number(基准值) < 1.005 && Number(基准值) > 1.004) {
      ...
      else if (Number(基准值) < 1.005 && Number(基准值) > 1.004) {

        if (Number(當前價格) < Number(布林下*1.0002) && Number(當前價格) > Number(布林下 / 1.1)) {
            log("當前價格= " + 當前價格);
          toastLog("當前價格符合---- 做多")
